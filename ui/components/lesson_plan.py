...
# Add to ui/components/lesson_plan.py

def prepare_knowledge_aware_lesson_plan_prompt(
    subject: str, 
    grade_level: str, 
    timeframe: str, 
    duration: str, 
    ...
    """Prepare a knowledge-aware lesson plan prompt.
    ...
    Args:
        ...
        duration: Duration of lesson
        days_per_week: Days of the week
        specific_goals: Learning goals
        materials_list: Required materials
        accommodations_list: Additional accommodations
        selected_iep: Selected IEP
        knowledge_state: Student knowledge state
        
    Returns:
        Formatted prompt for lesson plan generation
    """
    # Format knowledge state for inclusion in prompt
    knowledge_state_text = "Current Knowledge State:\n"
    
    # Sort components by mastery level (lowest first)
    sorted_components = sorted(knowledge_state.items(), key=lambda x: x[1])
    
    for component_id, mastery in sorted_components:
        # Format component name for readability
        component_name = component_id.replace("kc_", "").replace("_", " ").title()
        
        # Add mastery level description
        if mastery >= 0.8:
            level = "Proficient"
        elif mastery >= 0.6:
            level = "Developing"
        ...
        knowledge_state_text += f"- {component_name}: {mastery*100:.1f}% ({level})\n"
    
    # Identify focus areas (components with lowest mastery)
    focus_areas = [component_id.replace("kc_", "").replace("_", " ").title() 
                  for component_id, mastery in sorted_components[:3] if mastery < 0.8]
    
    if focus_areas:
        knowledge_state_text += "\nFocus Areas:\n"
        for area in focus_areas:
            knowledge_state_text += f"- {area}\n"
    ...
    Please prioritize skills with lower mastery levels (below 70%) while maintaining practice of mastered skills. Include appropriate scaffolding and accommodations based on the IEP and knowledge state.
    ...
    Please create a comprehensive lesson plan with:
    1. Learning objectives aligned with current knowledge state
    2. Detailed schedule/timeline
    3. Teaching strategies with specific IEP accommodations and targeted activities
    4. Assessment methods appropriate to the student's needs
    5. Resources and materials organization
...
def handle_lesson_plan_generation(
    ...
    specific_goals: str, 
    materials: str, 
    additional_accommodations: str,
    selected_iep_id: str, 
    selected_student_id: Optional[str],
    app_components: Dict[str, Any]
):
    """Handle lesson plan generation with knowledge state integration.
    ...
    Args:
        ...
        additional_accommodations: Additional accommodations
        selected_iep_id: Selected IEP ID
        selected_student_id: Optional student ID
        app_components: Dictionary with application components
    """
    try:
        # Find selected IEP
        ...
        # Prepare input data
        goals_list = specific_goals.strip().split("\n") if specific_goals else []
        materials_list = materials.strip().split("\n") if materials else []
        accommodations_list = additional_accommodations.strip().split("\n") if additional_accommodations else []
        
        # Prepare prompt based on whether student knowledge is available
        prompt = ""
        knowledge_state = {}
        
        if selected_student_id and "student_profile_manager" in app_components:
            # Get student knowledge state
            profile_manager = app_components["student_profile_manager"]
            knowledge_state = profile_manager.get_student_knowledge_state(selected_student_id, subject)
            
            if knowledge_state:
                # Create knowledge-aware prompt
                prompt = prepare_knowledge_aware_lesson_plan_prompt(
                    subject, grade_level, timeframe, duration, days_per_week,
                    goals_list, materials_list, accommodations_list,
                    selected_iep, knowledge_state
                )
            else:
                # Use standard prompt if no knowledge state available
                prompt = prepare_lesson_plan_prompt(
                    subject, grade_level, timeframe, duration, days_per_week,
                    goals_list, materials_list, accommodations_list,
                    selected_iep
        ...
        plan_data = {
            ...
            "includes_knowledge_state": bool(knowledge_state),
            "timestamp": datetime.now().isoformat()
        }
